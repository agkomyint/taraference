...
```text
  ...
  │  3. BASELINE profile → save numbers (0.5B / 3B)                   │
  │  4. CHANGE code on laptop (new method / kernel / decode)          │
  │  5. SMOKE-TEST on laptop (build + short run if GPU available)     │
  │  6. PUSH tag → Release CI builds Linux binary                     │
  │  7. WAIT for green CI + GitHub Release assets                     │
  │  8. SSH: tarafer update → re-profile vs baseline                  │
...
# optional for heavier A/B:
tarafer --download 3b
```

Keep model paths stable so profiles are comparable run-to-run.

---

## Phase 3 — Baseline: see the result (before changing code)

Always establish a **baseline** on the SSH GPU with the **current release**.

```bash
cd ~/work   # any dir; profile-logs/ is written relative to cwd
export PATH="$HOME/.local/bin:$PATH"

# Primary A/B model for speed iteration (cheap, high signal)
tarafer ~/models/Qwen2.5-0.5B-Instruct-Q4_K_M.gguf --profile --decode fastv2

# Optional: quality / heavier model (same methodology)
...
- `profile-logs/index.csv` (one row per run)
- GPU name + compute_cap in a note (“baseline T4 sm_75, release vX.Y.Z”)

**Do not** mix 0.5B and 3B when declaring a win. Compare only `latest_<same-model-stem>.txt`.

Example baseline you already measured on Lightning T4 (illustrative):

| Model | overall decode | drop first→last | VRAM peak |
|-------|---------------:|----------------:|----------:|
| 0.5B Q4 | ~102 tok/s | ~37% | ~0.6 GiB |
...
# if laptop has NVIDIA + CUDA 13.x:
.\target\release\tarafer.exe models\Qwen2.5-0.5B-Instruct-Q4_K_M.gguf --prompt "hi" -n 32
# optional local profile (numbers are for *this* GPU, not T4)
.\target\release\tarafer.exe models\...\gguf --profile --decode fastv2
```
...
| Compare | How |
|---------|-----|
| Same model stem | `latest_Qwen2.5-0.5B-Instruct-Q4_K_M.txt` vs previous |
| Same decode family | e.g. both `fastv2`, or A/B `fastv2` vs `fastv3` |
...
```bash
# quick look
grep -E 'overall_decode|decode_drop|decode_tps_' profile-logs/latest_*.txt
cat profile-logs/index.csv
```
...
```text
ssh → nvidia-smi → curl release → tarafer install → --download 0.5b → --profile → save baseline
```

### Every performance experiment

```text
laptop: implement → build/smoke
git tag vX.Y.Z → push → wait CI green
ssh: tarafer update --install → --profile → compare SUMMARY_KV → keep or iterate
```
...
| SSH | `tarafer --download 0.5b` |
| SSH | `tarafer <model.gguf> --profile --decode fastv2` |
| SSH | `tarafer <model.gguf>` (chat) or `--serve` |
| Laptop | `cargo build --release -p taraference` |
| Laptop | `tarafer.exe <model> --prompt "hi" -n 32` |
...
4. Comparing 0.5B vs 3B as if one score.  
5. Changing model, decode backend, *and* code in one A/B without isolating variables.  
...
- [ ] SSH: `--profile` same model as baseline  
